...
async function init() {
    ...
    // 获取模板, 优先使用全称 template, 其次使用简称 t
    const argTemplate = argv.template || argv.t

    let targetDir = argTargetDir || defaultTargetDir
    // 如果 targetDir 是 '.'，那么返回当前目录的名称；否则，返回 targetDir。
 ...
 * 通过删除尾斜杠来格式化目标目录。
 *
 * @param targetDir - The target directory to format.
 * @returns The formatted target directory.
 */
...
/**
 *将文件或目录从源路径复制到目标路径。
 *如果源是一个目录，它将递归复制所有文件和子目录。
 *如果源文件是一个文件，它会将文件复制到目标路径。
 *
 * @param src - The source path of the file or directory to copy.
 ...
 * @returns A boolean indicating whether the project name is valid or not.
 */
